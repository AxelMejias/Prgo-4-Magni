...
import type { InsumoFaltante } from "../types";
...
export default function CheckoutPage() {
  ...
  // Estado del popup de MP
  const [esperandoPago, setEsperandoPago] = useState(false);
  const [mpPedidoId, setMpPedidoId] = useState<number | null>(null);
  ...
  // Polling + storage listener mientras el usuario paga en el popup de MP
  useEffect(() => {
    if (!esperandoPago || !mpPedidoId) return;

    let cancelled = false;

    // El popup escribe a localStorage cuando MP redirige a pedido-exitoso;
    // el storage event solo dispara en ventanas DISTINTAS a la que escribe.
    function handleStorage(e: StorageEvent) {
      if (e.key !== "mp_pago_resultado" || !e.newValue) return;
      try {
        ...
        if (result.pedido_id !== mpPedidoId) return;
        cancelled = true;
        localStorage.removeItem("mp_pago_resultado");
        mpPopupRef.current?.close();
        clearCart();
        navigate(
          `/pedido-exitoso?collection_status=${result.status}` +
          `&external_reference=${result.pedido_id}` +
    ...
    // Polling al backend cada 2.5 s como respaldo
    const pollId = setInterval(async () => {
      if (cancelled) return;
      try {
        const result = await pedidoApi.verificarPago(mpPedidoId);
        if (result.status === "approved") {
          cancelled = true;
          mpPopupRef.current?.close();
          clearCart();
          navigate(
            `/pedido-exitoso?collection_status=approved` +
            `&external_reference=${mpPedidoId}` +
            `&payment_id=${result.payment_id ?? ""}`
          );
        } else if (result.status === "rejected" || result.status === "cancelled") {
          cancelled = true;
          mpPopupRef.current?.close();
          navigate(`/pedido-exitoso?collection_status=failure&external_reference=${mpPedidoId}`);
        }
      } catch {
    ...
    // Detectar si el usuario cerró el popup sin completar el pago
    const popupCheckId = setInterval(() => {
      if (mpPopupRef.current?.closed) {
        ...
        setTimeout(() => {
          if (!cancelled) {
            ...
            setMpPedidoId(null);
            setFormError(
              "Cerraste la ventana de MercadoPago. Tu pedido fue creado — podés completar el pago desde Mis Pedidos."
            );
          }
        }, 800);
      }
  ...
  }, [esperandoPago, mpPedidoId, navigate, clearCart]);
  ...
  const crearPedidoMutation = useMutation({
    ...
    onSuccess: (pedido) => {
      if (pedido.init_point) {
        ...
        const pw = 920, ph = 700;
        // Abrimos /mp-checkout-redirect primero (mismo origen) para que
        // sessionStorage marque la pestaña como popup antes de ir a MP.
        const redirectUrl =
          `/mp-checkout-redirect?url=${encodeURIComponent(pedido.init_point)}`;
        const popup = window.open(
          redirectUrl,
          "mp-checkout",
          `width=${pw},height=${ph},` +
          `left=${Math.round((sw - pw) / 2)},top=${Math.round((sh - ph) / 2)},` +
          `resizable=yes,scrollbars=yes`
        );
        mpPopupRef.current = popup;
        setMpPedidoId(pedido.id);
    ...
    onError: (err: unknown) => {
      // FastAPI devuelve: { detail: { code, detail, faltantes?, ... } }
      type ApiDetail = { code?: string; detail?: string; faltantes?: InsumoFaltante[] };
      const axiosErr = err as { response?: { data?: { detail?: ApiDetail } } };
      ...
      } else if (code === "PRODUCTO_NO_DISPONIBLE") {
        setFaltantes([]);
        setFormError(detail?.detail ?? "Uno o más productos no están disponibles.");
      } else {
        setFaltantes([]);
        setFormError(
          detail?.detail ?? (err as Error).message ?? "Error al crear el pedido."
        );
      }
    },
  });
  ...
  if (items.length === 0 && !esperandoPago) {
    return (
  ...
  return (
    <>
      {/* ── Overlay: esperando que el usuario complete el pago en el popup de MP ── */}
      {esperandoPago && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center space-y-5 shadow-2xl">
            <div className="text-5xl">💳</div>
            <div>
              <h2 className="text-xl font-bold text-surface-900">Completá tu pago</h2>
              <p className="text-sm text-surface-500 mt-1">
                Terminá el pago en la ventana de MercadoPago.
              </p>
            </div>
            <div className="flex items-center justify-center gap-2 text-brand-600 py-1">
              <svg
            ...
            <p className="text-xs text-surface-400">
              Esta página se actualizará automáticamente al confirmar el pago.
            </p>
            <button
              type="button"
              onClick={() => mpPopupRef.current?.focus()}
              className="text-sm text-brand-600 font-medium underline hover:text-brand-800 transition-colors"
            >
              Ir a la ventana de pago
            </button>
          </div>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={crearPedidoMutation.isPending || esperandoPago}
            ...
            >
              {crearPedidoMutation.isPending
                ? (formaPago === "MERCADOPAGO" ? "Creando pago…" : "Procesando pedido…")
                : formaPago === "MERCADOPAGO"
                ? `Pagar con MercadoPago — ${formatARS(total)}`
